...
server.tool("get_link_analytics", {
...
- Recent Clicks: ${link.clicks.slice(0, 5).map((click) => `${new Date(click.occurredAt).toLocaleString()}: ${click.ip || 'Unknown IP'} (${click.country || 'Unknown'}, ${click.city || 'Unknown'})`).join('\n')}`;
        return {
            content: [
                {
                    type: "text",
...
server.tool("list_user_links", {
    page: z.number().min(1).default(1).describe("Page number"),
    limit: z.number().min(1).max(100).default(10).describe("Number of links per page"),
}, async ({ page, limit }) => {
    try {
        ...
        const linksList = links.map((link) => `- ${link.slug}: ${link.destination} (${link.title || 'No title'}) - ${link.createdAt}`).join('\n');
        const summary = `Your Links (Page ${pagination.page}/${pagination.pages}, Total: ${pagination.total}):\n${linksList}`;
        return {
            content: [
                {
                    type: "text",
...
server.tool("bulk_create_links", {
    ...
    try {
        ...
        const summary = `Bulk creation completed! Created ${createdLinks.length} links:\n` +
            createdLinks.map((link) => `- ${link.slug}: ${link.shortUrl}`).join('\n');
        return {
            content: [
                {
                    type: "text",
...
server.tool("get_global_analytics", {
    ...
    try {
        ...
        analytics.timeBasedAnalytics.dailyStats.forEach((stat) => {
            summary += `${stat.date}: ${stat.count} requests\n`;
        });
        // Hourly stats
        summary += "\nHourly Request Trends (Last 24h):\n";
        analytics.timeBasedAnalytics.hourlyStats.forEach((stat) => {
            summary += `${stat.hour}:00: ${stat.count} requests\n`;
        });
        // Top links
        summary += "\nTop Performing Links:\n";
        analytics.topLinks.slice(0, 5).forEach((link, index) => {
...
server.tool("list_all_links", {
    page: z.number().min(1).default(1).describe("Page number"),
    limit: z.number().min(1).max(100).default(20).describe("Number of links per page"),
    ...
    try {
        ...
        const links = response.data.links;
        const pagination = response.data.pagination;
        const linksList = links.map((link) => `- ${link.slug}: ${link.destination} (${link.title || 'No title'}) - ${link.clicksCount} clicks - Created: ${new Date(link.createdAt).toLocaleDateString()}`).join('\n');
        const summary = `All Links (Page ${pagination.page}/${pagination.pages}, Total: ${pagination.total}):\n${linksList}`;
        return {
            content: [
                {
                    type: "text",
...
server.tool("manage_users", {
    ...
    try {
        ...
        if (action === "list") {
            ...
            const usersList = users.map((user) => `- ${user.email} (${user.name || 'No name'}) - Admin: ${user.isAdmin} - API Keys: ${user._count.apiKeys}`).join('\n');
            return {
                content: [
                    {
                        type: "text",
